  ...
  static void addUser(UserModel user) async {
    checkUserExist(user.userID).then((value) {
      if (!value) {
        ...
        Firestore.instance
            .document("${Const.usersCollection}/${user.userID}")
            .setData(user.toJson());
      } else {
  ...
  static Future<bool> checkUserExist(String userID) async {
    ...
    try {
      await Firestore.instance
          .document("${Const.usersCollection}/$userID")
          .get()
          .then((doc) {
        if (doc.exists)
          exists = true;
...
//          return 'User with this e-mail not found.';
          return 'Người dùng với e-mail này không tìm thấy.';
          break;
...
//          return 'Unknown error occured.';
          return 'Xảy ra lỗi không xác định được.';
      }
    } else {
      return 'Xảy ra lỗi không xác định được.';
    }
